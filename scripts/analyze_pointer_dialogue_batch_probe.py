...
def classify(probe_path: Path, boot_path: Path, candidate_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "reason": (
            "Known opening route did not reach pointer 2/3 before the hard frame cap."
...
def main() -> int:
    ...
    payload = classify(args.probe, args.boot, args.candidate)
