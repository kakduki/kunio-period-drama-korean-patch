...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="manifest_plan_test_") as temp:
        ...
        output = make_manifest_plan(source, {182: "\ucfe0\ub2c8\ub9c8\uc0ac \uc5b4\uc11c"}, root)
        result = json.loads(output.read_text(encoding="utf-8"))
    assignments = result["pointer_page_assignments"]
    assert assignments[182] == 48
    assert all(value is None for index, value in enumerate(assignments) if index != 182)
    assert result["optimized_pages"][48]["syllables"] == ["\ub2c8", "\ub9c8", "\uc0ac", "\uc11c", "\uc5b4", "\ucfe0"]
