-- Temporary-state inventory probe for the v0.4.2 Katana visual target.
--
-- This does not patch ROM or save files. It writes the likely Katana low item
-- id (0x84) into narrow RAM/SRAM candidate slots before opening the item list,
-- then dumps changed screens so we can see whether the label renderer appears.
...
KUNIO_MANUAL_DUMP_OUTPUT = os.getenv("KUNIO_ANALYSIS_OUTPUT") or (ROOT_DIR .. "/rom_analysis/katana_inventory_probe_v042")
...
local summary_path = KUNIO_MANUAL_DUMP_OUTPUT .. "/inventory_probe_summary.tsv"
...
local function inject_katana_candidates()
    local candidates = {
        0x0502, 0x0503, 0x0506, 0x0508, 0x0509,
        0x0700, 0x0701, 0x0702, 0x0706, 0x0707, 0x071D, 0x071E, 0x071F,
        0x7700, 0x7701, 0x7705, 0x7720, 0x7721, 0x7722,
    }
    for _, addr in ipairs(candidates) do
        write_byte(addr, 0x84)
    end
end

mkdir(KUNIO_MANUAL_DUMP_OUTPUT)
append(summary_path, "frame\tphase\tfingerprint\tdump_prefix")
...
while emu.framecount() < MAX_FRAMES do
    ...
    if frame >= 1500 and frame <= 2320 then
        inject_katana_candidates()
    end
    local buttons = input_for(frame)
    joypad.set(1, buttons)
    gui.text(2, 8, "Kunio Katana inventory probe")
    gui.text(2, 17, "frame=" .. tostring(frame) .. " inject=0x84")
    emu.frameadvance()
...
append(summary_path, table.concat({ tostring(emu.framecount()), "done", tostring(last_fingerprint or ""), "" }, "\t"))
