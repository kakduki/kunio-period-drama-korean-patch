...
For the current seven broad-scan proof candidates, prefer the route wrappers:
...
Each route wrapper shows the active route and screen hint in the FCEUX overlay.
If it still shows only the title/opening screen, stop with `Q` and manually
change screens instead of waiting.

For multiple broad-scan screens in one play session when the route is unclear,
run:
