...
| full_pointer_korean_candidate | pointers `0-247`; packed records `0x05FC4-0x06EF0` (end-exclusive); 49 appended CHR pages | all active records preserve English non-letter control skeletons; width audit max `24/24`; 83 rows reviewed; 4 excluded rows retain Japanese bytes | forced pointers 0/25/50/100/181 PASS across pages 11/16/41/46/42; state, R1, source, terminator PASS | Korean text pixels and field background PASS on all 5 samples | WHOLE_SCRIPT_RUNTIME_PASS_5_PAGES |
...
`146d9da994c933a98649bb0f631ee3a3`. It compiles 244 Korean dialogue rows,
including 83 English-reference-reviewed rows, retains four excluded
...
event-control proof; 161 draft rows and broad visual coverage remain open.
