...
| full_pointer_korean_candidate | pointers `0-247`; packed records `0x05FC4-0x06EBB` (end-exclusive); 48 appended CHR pages | all active records preserve English non-letter control skeletons; 10 token-sensitive records use explicit Korean segments; width audit max `20/24`; 240 rows reviewed; 4 excluded rows retain Japanese bytes | forced pointers 0/25/50/110/181 PASS across pages 10/14/40/31/42; state, R1, source, terminator PASS | Korean text pixels and field background PASS on all 5 samples | WHOLE_SCRIPT_RUNTIME_PASS_5_PAGES |
...
`7187caa40a2761d0b373566e9cfb142d`. It compiles 244 Korean dialogue rows,
...
event-control proof; four incomplete-source draft rows and broad visual coverage remain open.
