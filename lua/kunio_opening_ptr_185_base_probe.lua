-- Bounded context probe for the fourth opening dialogue record.
--
-- This follows only the already-proven title/menu route, acknowledges the
-- first three opening dialogue records once each, and stops as soon as
-- pointer 185 is read and captured. It never enters free-form gameplay.

local OUT_DIR = os.getenv("KUNIO_ANALYSIS_OUTPUT") or "rom_analysis/opening_ptr_185_base_probe_capture"
local TARGETS_LUA = os.getenv("KUNIO_TARGETS_LUA") or "kunio_opening_ptr_185_base_target.lua"
...
local function on_read_for(target)
    return function(addr, size, value)
        ...
        append(reads_path, table.concat({
            emu.framecount(), target.label, "$" .. hex4(addr or 0),
            hex2(value or byte_at(addr or 0)), tostring(matches), record_snapshot(target),
        }, "\t"))
...
local function capture(target)
    ...
    append(record_path, table.concat({
        frame, target.label, "$" .. hex4(target.start) .. "-$" .. hex4(target.stop),
        target.bytes, tostring(record_matches(target)), record_snapshot(target),
    }, "\t"))
    append(summary_path, table.concat({
        frame, "capture", registered_count, hit_count,
...
for addr = target.start, target.stop do
    if register_read(addr, on_read_for(target)) then registered_count = registered_count + 1 end
end
...
append(summary_path, table.concat({
    ...
    "advance=" .. FIRST_ADVANCE_START .. "-" .. FIRST_ADVANCE_END .. "," ..
    SECOND_ADVANCE_START .. "-" .. SECOND_ADVANCE_END .. "," ..
    THIRD_ADVANCE_START .. "-" .. THIRD_ADVANCE_END .. ";target=" .. target.label,
}, "\t"))

while emu.framecount() < MAX_FRAMES and not captured and not stopped_for_limit do
    local frame = emu.framecount()
    joypad.set(1, route_input(frame))
    gui.text(2, 8, "Opening pointer 185 probe frame " .. tostring(frame))
...
append(summary_path, table.concat({
    emu.framecount(), final_reason, registered_count, hit_count,
