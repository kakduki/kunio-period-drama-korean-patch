-- Bounded context probe for the opening dialogue record immediately after
-- pointer 182. This runs the known opening route, presses B once to advance
-- the verified first dialogue, and captures only after pointer 183 is read.
-- It never enters free-form gameplay or keeps pressing through later scenes.

local OUT_DIR = os.getenv("KUNIO_ANALYSIS_OUTPUT") or "rom_analysis/opening_ptr_183_base_probe_capture"
local TARGETS_LUA = os.getenv("KUNIO_TARGETS_LUA") or "kunio_opening_ptr_183_base_target.lua"
local MAX_FRAMES = tonumber(os.getenv("KUNIO_MAX_FRAMES") or "1180")
local HIT_LIMIT = tonumber(os.getenv("KUNIO_HIT_LIMIT") or "5000")
local ADVANCE_START_FRAME = tonumber(os.getenv("KUNIO_ADVANCE_START_FRAME") or "900")
local ADVANCE_END_FRAME = tonumber(os.getenv("KUNIO_ADVANCE_END_FRAME") or "910")
...
local function route_input(frame)
    -- This is the previously verified route to the frame-883 opening dialogue.
    ...
    -- One deliberate acknowledgement of pointer 182; no further inputs are
    -- sent, so a failed probe cannot turn into open-ended autoplay.
    if frame >= ADVANCE_START_FRAME and frame < ADVANCE_END_FRAME then return { B = true } end
    return {}
end
...
append(summary_path, table.concat({
    ...
    registered_count,
    0,
    "advance=" .. tostring(ADVANCE_START_FRAME) .. "-" .. tostring(ADVANCE_END_FRAME) .. ";target=" .. target.label,
}, "\t"))

while emu.framecount() < MAX_FRAMES and not captured and not stopped_for_limit do
    local frame = emu.framecount()
    joypad.set(1, route_input(frame))
    gui.text(2, 8, "Opening pointer 183 probe frame " .. tostring(frame))
