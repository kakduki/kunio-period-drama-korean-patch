...
- `font_expansion_readiness.md` / `font_expansion_readiness.json`: checks planned glyph batches against the current `font/char_map.json` and `font/korean_font_8x16.bin` assets before attempting larger font expansion builds
- `batch46_text_readiness.md` / `batch46_text_readiness.json`: reclassifies the broad-scan capture queue after the largest currently buildable font expansion
...
- `kunio_period_drama_korean_font_expansion_v0.5_batch46_report.md` / `.json`: local font-only expansion build report for the largest currently buildable prefix from the existing font assets
- `translation_scan_capture_queue.md` / `translation_scan_capture_queue.json`: supplemental manual capture queue for new high/medium confidence hits from the broad translation scan, with readable labels joined from the translation reference
- `reference_capture_plan.md` / `reference_capture_plan.json`: transcript/reference-guided manual screen priorities; use this to choose concrete screens without treating video/transcription order as ROM proof
- `video_route_reference.md`: gameplay-video route reference for replacing blind autoplay with a known path
- `broad_scan_patchability.md` / `broad_scan_patchability.json`: filters the broad-scan queue down to length-safe future candidates, lists v0.4.2 planned PRG bytes, and separates compact-from-base glyph slots from the actual v0.4.2 byte map
...
- `translation_scan_capture_queue.md` turns those broad-scan hits into a conservative supplemental manual-capture list. It currently queues 60 hits: 6 high confidence, 33 medium confidence, and 21 low confidence. These are verification targets, not approved patch offsets. The queue joins readable Japanese/Korean labels and screen hints from `translation_readable_reference.json` so it can be used without reading mojibake-only candidate strings.
- `reference_capture_plan.md` ranks the capture queue using the transcription/reference section order and patchability data. It is a navigation aid for finding real screens in FCEUX or a gameplay video; it does not promote offsets without CPU-read and visual-context proof.
...
- `font_expansion_readiness.md` shows that the next 32 planned glyphs are buildable from current font assets, but larger planned batches are gated by missing current font/char-map entries. The maximum prefix buildable from the current assets is 46 extra glyphs; batch64 is the first blocked batch, so do not spend time trying larger font expansion builds until the font assets are extended or regenerated.
- `kunio_period_drama_korean_font_expansion_v0.5_batch46_report.md` records that the 46-glyph font-only candidate writes 64 total glyph slots, changes 837 bytes, and keeps all changed bytes inside CHR Bank 07. It is still only a font coverage candidate; text promotion remains gated by ROM/runtime/screen proof.
- `batch46_text_readiness.md` confirms that batch46 does not add new broad-scan screen-proof candidates: the queue stays at 27 font-ready rows and 7 screen-proof candidates. The next useful bottleneck is therefore manual CPU-read/visual proof for those 7 candidates, not more blind font-only expansion.
- `manual_proof_routes.md` reduces those 7 screen-proof candidates to 3 manual route groups: Kajiya (`0x0440C`), Tatsuji (`0x048F4`, `0x052A5`, `0x05BE5`), and Heishichi (`0x06294`, `0x0631B`, `0x06359`). Use these groups to capture real screens efficiently instead of treating each offset as a separate playthrough.
- `route_fceux_targets.md` turns those 3 groups into route-specific watcher scripts: `lua/kunio_manual_route_kajiya_capture_watch.lua`, `lua/kunio_manual_route_tatsuji_capture_watch.lua`, and `lua/kunio_manual_route_heishichi_capture_watch.lua`. Use these instead of the all-target broad watcher when you know which screen group you reached.
