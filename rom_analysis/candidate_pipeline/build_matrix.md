# Build Matrix

Development builds use a soft gate. Release approval uses the separate hard gate checklist.

| build id | scope | source screen/context | ROM offset | PRG bank | patch type | build | boot smoke | visual proof | decision |
| --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- |
| `softgate-0562f-tatsuichi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0562F` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-05643-heishichi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x05643` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-0561a-hashi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0561A` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-0569d-hashi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0569D` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-056da-hashi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x056DA` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-0571c-hashi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0571C` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-057d4-hashi` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x057D4` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-0736a-raifu` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0736A` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-0739d-raifu` | one string | `fceux_input_explorer_v042 frame 883 dialogue screen` | `0x0739D` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | candidate ROM produced |
| `softgate-dev-combined` | 9 strings | `fceux_input_explorer_v042 frame 883 dialogue screen` | `multiple` | 1 | cumulative equal-length PRG bytes + existing font expansion | PASS | PASS | soft gate only | cumulative dev candidate produced |
| `softgate-quarantine-05644-katana` | quarantined one string | `item-menu/runtime watch evidence; visual context pending` | `0x05644` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | required before release/dev merge | HIGH_RISK_OVERLAPS_SELECTED_DEV_SPAN |
| `softgate-quarantine-06295-katana` | quarantined one string | `item-menu/runtime watch evidence; visual context pending` | `0x06295` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | required before release/dev merge | HIGH_RISK_VISUAL_CONTEXT_PENDING |
| `softgate-quarantine-0631c-katana` | quarantined one string | `item-menu/runtime watch evidence; visual context pending` | `0x0631C` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | required before release/dev merge | HIGH_RISK_VISUAL_CONTEXT_PENDING |
| `softgate-quarantine-0635a-katana` | quarantined one string | `item-menu/runtime watch evidence; visual context pending` | `0x0635A` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | required before release/dev merge | HIGH_RISK_VISUAL_CONTEXT_PENDING |
| `softgate-quarantine-07227-katana` | quarantined one string | `item-menu/runtime watch evidence; visual context pending` | `0x07227` | 1 | equal-length PRG bytes + existing font expansion | PASS | PASS | required before release/dev merge | HIGH_RISK_WRONG_CONTEXT_REPRODUCED |

## Notes

- `softgate-0562f-tatsuichi` source string: `たついち` / `타츠이치` / Tatsuichi
- `softgate-05643-heishichi` source string: `へいしち` / `헤이시치` / Heishichi
- `softgate-0561a-hashi` source string: `はし` / `다리` / Hashi
- `softgate-0569d-hashi` source string: `はし` / `다리` / Hashi
- `softgate-056da-hashi` source string: `はし` / `다리` / Hashi
- `softgate-0571c-hashi` source string: `はし` / `다리` / Hashi
- `softgate-057d4-hashi` source string: `はし` / `다리` / Hashi
- `softgate-0736a-raifu` source string: `ライフ` / `라이프` / Raifu
- `softgate-0739d-raifu` source string: `ライフ` / `라이프` / Raifu
- `softgate-quarantine-05644-katana` quarantined source string: `カタナ` / `카타나` / Katana
- `softgate-quarantine-06295-katana` quarantined source string: `カタナ` / `카타나` / Katana
- `softgate-quarantine-0631c-katana` quarantined source string: `カタナ` / `카타나` / Katana
- `softgate-quarantine-0635a-katana` quarantined source string: `カタナ` / `카타나` / Katana
- `softgate-quarantine-07227-katana` quarantined source string: `カタナ` / `카타나` / Katana
- Source screenshot: `rom_analysis/fceux_input_explorer_v042/manual_frame_000883_screen.png`
- This matrix intentionally does not require manual visual proof for development candidates.

## English-Patch-Aligned Candidate

| build id | scope | source screen/context | ROM offset | PRG bank | patch type | build | boot smoke | visual proof | decision |
| --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- |
| direct-low-120-labels | 120 direct-low labels | English TSe-v10.ips direct-low 8x8 renderer rows across UI/item/status contexts | 0x07897-0x1CD08 | 1, 3, 4, 6, 7 | same renderer byte contract + CHR Bank 7 glyph allocation + full base-to-candidate IPS | PASS | PASS | UNKNOWN per context | soft-gate candidate produced; not release |

- Candidate MD5: 2c012fbb98bc93885f884a111839bcf3.
- The builder covered 120/120 extracted direct-low rows and allocated 77 existing-font glyphs to tiles 0x101-0x14D.
- The candidate IPS contains 493 records and applies from the clean Japanese base ROM with round-trip PASS.
- Bounded menu and gameplay-entry smoke passed; individual direct-low screen ownership remains UNKNOWN until each relevant menu/item/status context is captured.

\n